$(document).ready(() => {
    ...
    function manageData() {
        $.ajax({
            dataType: 'json',
            url: "../ajaxfile/teacherAction.php",
            type: "post",
            data: { status: 'fetch' },
            success: function (data) {
                // console.log(data);
    ...
    $(document).on('click', '.tbl-edit', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                // Skill check setup
                var skill1 = []
                var skill = val.skill.split(",")
                for (var i = 0; i < skill.length; i++) {
                    skill1.push(skill[i])
                }
                $("#myModalEdit #skill option").each(function () {
                    var valu = $(this).val()
                    if (inArray(valu, skill1)) {
                        $(this).attr({ selected: true, style: "color:aliceblue;background-color: darkgray;" })
                    } else {
    ...
    $("#update_btn").click(function (e) {
        ...
        if (blnkinput("#myModalEdit #zip")) {
            errors[5] = $("#myModalEdit #zipMsg").attr("class", "danger").text("郵便番号は空白にしないでください。")
            clearMsg("#myModalEdit #zip")
        } else if (!zipPattr.test(zip)) {
            errors[5] = $("#myModalEdit #zipMsg").attr("class", "danger").text("郵便番号は7桁のみにする必要があります。");
            clearMsg("#myModalEdit #zip")
        }
        ...
        if (blnkinput("#myModalEdit #skill")) {
            errors[11] = $("#myModalEdit #skillMsg").attr("class", "danger").text("スキルは空白にしないでください。")
            $("#myModalEdit #skill").on("input", () => {
                $("#myModalEdit #skillMsg").text("")
            })
        }
        if (!$('#myModalEdit input[name="hobby[]"]').is(":checked")) {
            // if ($(".hob:checked").val() == null) {
            errors[12] = $("#myModalEdit #hobbyMsg").attr("class", "danger").text("趣味は空白にしないでください。")
        ...
        var skillVal = $("#myModalEdit #skill").val();
        $.each(skillVal, function () {
            selectedSkill.push(this);
        });
    ...
    $("#addForm").on("submit", function (e) {
        ...
        if (blnkinput("#zip")) {
            errors[5] = $("#zipMsg").attr("class", "danger").text("郵便番号は空白にしないでください。")
            clearMsg("#zip")
        } else if (!zipPattr.test(zip)) {
            errors[5] = $("#zipMsg").attr("class", "danger").text("郵便番号は7桁のみにする必要があります。");
            clearMsg("#zip")
        }
        ...
        if (blnkinput("#skill")) {
            errors[11] = $("#skillMsg").attr("class", "danger").text("スキルは空白にしないでください。")
            $("#skill").on("input", () => {
                $("#skillMsg").text("")
            })
        }
        if (!$('input[name="hobby[]"]').is(":checked")) {
            // if ($(".hob:checked").val() == null) {
            errors[12] = $("#hobbyMsg").attr("class", "danger").text("趣味は空白にしないでください。")
